...
html = html.replace(
    /onclick="if\(confirm\('Vuoi davvero eliminare questo corso\?'\)\)\{ app\.deleteCorso\(\${id}\); app\.closeModal\(\); \}"/,
    'onclick="app.promptDeleteCorso(${id})"'
);

const deleteCorsoRegex = /deleteCorso\(id\) \{[\s\S]*?this\.renderSalaPesi\(\);\s*\}\s*\},/;
...
if (deleteCorsoRegex.test(html)) {
    html = html.replace(deleteCorsoRegex, deleteCorsoFix);
    fs.writeFileSync('index.html', html);
    console.log('Successfully replaced delete logic.');
} else {
