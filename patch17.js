...
const editModalTarget = `onclick="if(confirm('Vuoi davvero eliminare questo corso?')){ app.deleteCorso(\${id}); app.closeModal(); }"`;
const editModalFix = `onclick="app.promptDeleteCorso(\${id})"`;

html = html.replace(editModalTarget, editModalFix);

const deleteCorsoTarget = `deleteCorso(id) {
            const index = this.state.corsi.findIndex(x => x.id === id);
            if (index > -1) {
                this.state.corsi.splice(index, 1);
                this.saveAll();
                this.renderSalaPesi();
            }
        },`;

const deleteCorsoFix = `promptDeleteCorso(id) {
            ...
            this.openModal('Opzioni Eliminazione', \`
                <p>Stai per eliminare il corso <strong>\${c.nome}</strong> del \${giorni[c.dayOfWeek % 7]}.</p>
                <p>Cosa vuoi fare?</p>
                <div style="display:flex; flex-direction:column; gap:12px; margin-top:20px;">
                    ...
                    <button class="btn" style="background:var(--card-bg); border:1px solid var(--warning); color:var(--text); text-align:left; padding:12px; display:flex; flex-direction:column; gap:4px;" onclick="app.executeDeleteCorso(\${id}, '\${targetDateStr}')">
                        <div style="font-weight:600; font-size:1rem; color:var(--warning);">✂️ Elimina SOLO in questa data</div>
                        <span style="font-size:0.8rem; color:var(--text-muted);">Il corso verrà interrotto questa settimana (\${targetDateFormatted}) e riprenderà la prossima.</span>
                    </button>
        ...
        executeDeleteCorso(id, mode) {
            ...
            if (mode === 'all') {
                ...
                if (!c.fine || c.fine >= nextWeekStr) {
                    ...
                    clone.iscritti = []; // svuotiamo gli iscritti per il corso futuro
                ...
                this.saveAll();
                this.toast('Giorno specifico eliminato con successo.');
                this.renderSalaPesi();
            }
        },
...
if(html.includes(editModalTarget)) {
    html = html.replace(editModalTarget, editModalFix);
} else {
    console.log("Edit target not found!");
}

if(html.includes(deleteCorsoTarget)) {
    html = html.replace(deleteCorsoTarget, deleteCorsoFix);
} else {
    console.log("Delete corso target not found!");
}

fs.writeFileSync('index.html', html);
console.log('Successfully applied patch to promptDeleteCorso.');
